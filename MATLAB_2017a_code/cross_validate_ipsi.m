...
nblocks = 1;  % number of blocks = number of points for single whisker
block_size=500; % block size = number of trials used for the fits
match = zeros(nblocks,2); % percentage of decision match
                    % COL 1 = LL
                    % COL 2 = NL-NL
...
for block=1:nblocks
    fit_decisions=zeros(tot_num_trials,2);  % col order is same as match 
    ...
    % lin_1    means L-L
    % lin_2    means L-NL
    h_nonlin_1 = true_h;%modes_nonlinnonlin(block);
    %h_nonlin_2 = %modes_nonlinlin(block);
    g_lin_1 = true_g;%modes_linlin(block);
